...
<script type="text/javascript">
                ...
                success:function (data) {
                    ...
                    */
                    if(data.success){
                        ...
                        html +='<font color="gray">客户</font> <font color="gray">-</font> <b>${c.name}</b> <small style="color: gray;" id="s'+data.cr.id+'"> '+(data.cr.editFlag==0?data.cr.createTime:data.cr.editTime)+' 由'+(data.cr.editFlag==0?data.cr.createBy:data.cr.editBy)+'</small>';
	...
	function showTransactionList() {
		$.ajax({
			url:"workbench/customer/getTransactionListByCid.do",
			data:{
				"id":"${c.id}"
			},
			type:"get",
			...
			success: function (data) {
				...
				$.each(data,function (i,n) {
					...
					html +='<td>'+n.type+'</td>';
					html +='<td><a href="javascript:void(0);"  onclick="removeTranModal(\''+n.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>';
					html +='</tr>';
				})
	...
	function showContactList() {
		$.ajax({
			url:"workbench/customer/getContactListByCid.do",
			data:{
				"id":"${c.id}"
			},
			type:"get",
			...
			success:function (data) {
				...
				$.each(data,function (i,n) {
					...
					html +='<td>'+n.email+'</td>';
					html +='<td>'+n.mphone+'</td>';
					html +='<td><a href="javascript:void(0);" onclick="removeContModal(\''+n.id+'\')" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>';
					html +='</tr>';
				})
		...
		<div class="modal-dialog" role="document" style="width: 30%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-danger" data-dismiss="modal" id="deleteBtn">删除</button>
				</div>
			</div>
		</div>
	</div>
    ...
    <div class="modal fade" id="removeTransactionModal" role="dialog">
        <div class="modal-dialog" role="document" style="width: 30%;">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-danger" data-dismiss="modal" id="removeBtn">删除</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <form class="form-horizontal" role="form">
                        ...
                        <div style="position: relative;top: 20px;">
                            <div class="form-group">
                                ...
                                <div class="col-sm-10" style="width: 81%;">
                                    <textarea class="form-control" rows="1" id="edit-address1">北京大兴大族企业湾</textarea>
                                </div>
                            </div>
                        </div>
                    </form>
